"""Test fixture for dual-region deployment with restricted APIs access disabled - private access will be enabled."""
...
FIXTURE_NAME = "mrpn-disable-restricted-apis"
...
def test_routes(routes_client: compute_v1.RoutesClient, project_id: str) -> None:
    ...
    assert len(private_apis_routes) == 1
    for route in private_apis_routes:
        assert route.name == f"{FIXTURE_NAME}-private-apis"
        assert route.description == "Route for private Google API access"
        assert (
