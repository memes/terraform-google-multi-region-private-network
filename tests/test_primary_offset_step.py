...
FIXTURE_NAME = "primary-offset-step"
FIXTURE_LABELS = {
    ...
    assert output == {
        ...
        "subnets_by_name": {
            f"{fixture_name}-us-we1": {
                ...
                "primary_ipv4_cidr": "172.16.10.0/24",
                "primary_ipv6_cidr": "",
            ...
            f"{fixture_name}-us-ea1": {
                ...
                "primary_ipv4_cidr": "172.16.20.0/24",
                "primary_ipv6_cidr": "",
                "secondary_ipv4_cidrs": {},
                "gateway_address": "172.16.20.1",
            },
        },
        "subnets_by_region": {
            "us-west1": {
...
def test_subnetwork_us_west1(
    ...
    assert result.ip_cidr_range == "172.16.10.0/24"
    ...
    assert len(result.secondary_ip_ranges) == 0
...
def test_subnetwork_us_east1(
    ...
    assert not result.external_ipv6_prefix
    assert not result.internal_ipv6_prefix
    assert result.ip_cidr_range == "172.16.20.0/24"
    assert not result.ipv6_cidr_range
    assert not result.log_config.enable
