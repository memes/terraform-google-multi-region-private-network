...
FIXTURE_NAME = "mrpn-primary-secondary-offset-step"
...
def output(
    ...
    with run_tofu_in_workspace(
        ...
        tfvars={
            ...
            "cidrs": {
                "primary_ipv4_cidr": "10.0.0.0/8",
                "primary_ipv4_subnet_size": 16,
                "primary_ipv4_subnet_offset": 7,
                "primary_ipv4_subnet_step": 7,
                "primary_ipv6_cidr": "null",
                "secondaries": {
                    "test": {
                        "ipv4_cidr": "192.168.0.0/16",
                        "ipv4_subnet_size": 24,
                        "ipv4_subnet_offset": 5,
                        "ipv4_subnet_step": 9,
                    },
                },
            },
    ...
    assert output == {
        ...
        "subnets_by_name": {
            ...
            f"{FIXTURE_NAME}-us-ce1": {
                ...
                "primary_ipv4_cidr": "10.14.0.0/16",
                "primary_ipv6_cidr": "",
                "secondary_ipv4_cidrs": {
                    "test": "192.168.14.0/24",
                },
...
def test_subnetwork_us_west1(subnetworks_client: compute_v1.SubnetworksClient, project_id: str) -> None:
    ...
    assert result.ip_cidr_range == "10.7.0.0/16"
    ...
    for secondary in result.secondary_ip_ranges:
        ...
        assert secondary.ip_cidr_range == "192.168.5.0/24"
...
def test_subnetwork_us_central1(subnetworks_client: compute_v1.SubnetworksClient, project_id: str) -> None:
    ...
    assert not result.external_ipv6_prefix
    assert not result.internal_ipv6_prefix
    assert result.ip_cidr_range == "10.14.0.0/16"
    assert not result.ipv6_cidr_range
    assert not result.log_config.enable
    ...
    for secondary in result.secondary_ip_ranges:
        assert secondary.range_name == "test"
        assert secondary.ip_cidr_range == "192.168.14.0/24"
    assert result.stack_type == "IPV4_ONLY"
    assert not result.state
