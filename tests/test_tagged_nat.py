"""Test fixture for dual-region deployment with Cloud NAT for tagged instances."""
...
def test_routes(routes_client: compute_v1.RoutesClient, project_id: str) -> None:
    ...
    assert len(tagged_routes) == 1
    for route in tagged_routes:
        assert route.name == f"{FIXTURE_NAME}-tagged-nat"
        assert route.description == "Route to NAT gateway for tagged resources"
        assert (
            route.next_hop_gateway
            == f"https://www.googleapis.com/compute/v1/projects/{project_id}/global/gateways/default-internet-gateway"
        )
        assert route.priority == 900  # noqa: PLR2004
        assert FIXTURE_NAME in route.tags
